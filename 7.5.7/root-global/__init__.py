# -*- coding: utf-8 -*-
# Copyright [2024-2025] [random9528]
...
def compare_and_download_files(base_folder, files):
    for _file in files:
        ...
        if os.path.isfile(full_path):
            digest = hashlib.md5(open(full_path, 'rb').read()).hexdigest()
            if digest != _file['hash']:
...
if __name__ != 'src':
    ...
    root.init(__name__, current_location, vendors)
